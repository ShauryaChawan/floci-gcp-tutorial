...
> 💡 **Better approach:** Group services by the problem they solve.
...
```mermaid
flowchart LR
    O[🛒 Order Service] -->|publish event| P[📨 Pub/Sub]
    P --> N[🔔 Notification Service]
    P --> A[📊 Analytics Service]
```
...
```mermaid
flowchart TD
    Q[🤔 Need to store user-uploaded images] --> P[📦 Object storage problem]
    P --> S[🪣 Cloud Storage]
```
...
```mermaid
flowchart TD
    Q[🤔 Several services need to react to order.created] --> P[📨 Messaging / eventing problem]
    P --> S[📨 Pub/Sub]
```
...
| 🗄️ Databases | Firestore, Datastore, Cloud SQL | Store application data |
...
> ✅ **Remember:** You do not need to memorize every service yet. The later chapters will introduce these services when there is a concrete problem for them to solve.
